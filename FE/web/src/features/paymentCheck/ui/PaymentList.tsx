...
export const PaymentList = () => {
  ...
  const [listShowState, setListShowState] = useState<'all' | 'in' | 'out'>('all');
  ...
  const filteredList =
    listShowState === 'all'
      ? paymentList.content.sort((a, b) => new Date(b.time).getTime() - new Date(a.time).getTime())
      : listShowState === 'in'
      ? paymentList.content
  ...
  return (
    <div className='flex flex-col gap-4'>
      <PaymentIntersection listShowState={listShowState} setListShowState={setListShowState} />
      <div className='space-y-4'>
        {Array.isArray(filteredList) && filteredList.length > 0 ? (
          filteredList.map((payment) => (
            <div key={payment.title} className='py-3 flex justify-between items-center'>
              <div>
              ...
              <div className={payment.in_out_amount > 0 ? 'text-blue-600' : 'text-red-600'}>
                {payment.in_out_amount.toLocaleString()}원
              </div>
            </div>
          ))
        ) : (
          <div className='text-center text-gray-500 py-4'>결제 내역이 없습니다.</div>
        )}
      </div>
    </div>
  );
